...
def handle_typing(event, current_text, inventory):
    if event.type == pygame.KEYDOWN:
        ...
        if char.isprintable() and char != "":
            ...
            elif char.islower(): can_type = True

            if inv_key:
                current_count = sum(1 for c in current_text if
                                    (inv_key == "uppercase" and c.isupper()) or
                                    (inv_key == "numbers" and c.isdigit()) or
                                    (inv_key == "special_characters" and c in string.punctuation))
                if current_count < inventory[inv_key]:
                    can_type = True
